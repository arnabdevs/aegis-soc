...
def _mask_target(target: str) -> str:
    """Mask domain or email for privacy in public dashboard."""
    if not target:
        return "***"
    if "@" in target:
        ...
        # Domain: example.com → ex***.com
        parts = target.rsplit(".", 1)
        masked = (parts[0][:2] + "***" + "." + parts[1]) if len(parts) == 2 else target[:2] + "***"
        return masked
...
def dashboard_stats():
    ...
    # Mask targets for privacy — never expose other users' domains/emails
    masked_scans = []
    for s_ in scans[:5]:
        masked_scans.append({
            "target":       _mask_target(s_.get("target", "")),
            "type":         s_.get("type", "—"),
            "health":       s_.get("health"),
            "threat_level": s_.get("threat_level", "LOW"),
            "time":         s_.get("time", "—"),
        })
